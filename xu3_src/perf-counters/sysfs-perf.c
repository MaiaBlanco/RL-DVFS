/*
	Description: Sysfs-interface function definitions for for perf-counters 
			...
			(specifically A-7, A-15, and A-53 in v7 mode).
				...
				are desired for the kernel module's
				functionality (and for them to show up in sysfs), then the 
...
static ssize_t cntr_attr_store(struct kobject* kobj, struct attribute *attr, 
 						const char* buf, size_t len);
static void cntr_release(struct kobject* kobj);
static ssize_t cntr_show(struct cpu_counter_obj* obj, struct cntr_attribute* attr, char* buf);
...
// Internal (statically defined) function implementations for sysfs interface:
/*
